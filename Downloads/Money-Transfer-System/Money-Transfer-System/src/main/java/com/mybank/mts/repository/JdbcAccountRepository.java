...
public class JdbcAccountRepository implements AccountRepository {
	...
	public Customer find(String acNo) {
		...
		try {
			...
			if (rs.next()) {
				cust = new Customer(rs.getString(2), rs.getString(1), rs.getInt(3));
			}else {
				return null;
			}
		} catch (SQLException e) {
	...
	//deletes the entire contents of the table customer
	public void clean() {
		Connection con = null;
		try {
			con = MySqlConnection.getMySqlconnection();
			String sql = "truncate table mybank_transact.customer";
			PreparedStatement ps = con.prepareStatement(sql);
			ps.executeUpdate();
		} catch (SQLException e) {
			e.printStackTrace();
